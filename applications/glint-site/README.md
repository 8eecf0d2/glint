...
The intro begins white, brings in five broad grey app windows in four tones, using overlapping desktop positions with small random offsets and consistent landscape proportions, then reveals the canonical logo. The logo grows in once without overshoot or pulsing, uses [Paper’s Liquid Metal shader](https://shaders.paper.design/liquid-metal) for flowing monochrome reflections within the canonical logo, then rises into place after a pause. The windows snap into place with uneven delays starting at 2.67 seconds; the settled composition holds for roughly a second before the logo rises at 4.45 seconds and content starts arriving at 4.85 seconds, overlapping the end of the logo lift. The renderer cues the page reveal, with a fallback to keep content accessible if WebGL fails. Paper Shaders 0.0.78 is pinned under Apache-2.0. The logo shader is loaded separately, pauses for reduced motion and hidden tabs, and stops after the intro; the canonical SVG remains underneath as its fallback.

A separate [Paper Mesh Gradient](https://shaders.paper.design/mesh-gradient) sits behind the windows at 32% opacity, using near-white cool, lilac and warm tones. It fades in after the white opening and drifts slowly with a 350,000-pixel rendering cap; reduced motion freezes it and hidden tabs pause it.
